package heven.holt.lib_common.widget.viewpager.utils;
...
public abstract class GracePagerAdapter<Item> extends PagerAdapter {
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        LogUtils.d("instantiateItem() called with: position = [" + position + "]");
    ...
    public void destroyItem(@NonNull ViewGroup container, int position, @NonNull Object object) {
        LogUtils.d("destroyItem() called with: position = [" + position + "]");
        ViewItemHolder viewItemHolder = (ViewItemHolder) object;
    ...
    public int getItemPosition(@NonNull Object object) {
        ...
        LogUtils.d("getItemPosition: oldPos=" + oldPos + ",newPos=" + newPos);
        if (itemPosition >= 0) {
            // 数据索引发生改变
    ...
    /**
     * 数据是否正在刷新中，即是否处于#notifyDataSetChanged()}到ViewPager#dataSetChanged(}执行过程
     *
     * @return 刷新中返回true
     */
