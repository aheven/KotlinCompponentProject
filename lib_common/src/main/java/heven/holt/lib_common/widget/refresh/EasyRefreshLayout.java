...
import heven.holt.lib_common.widget.refresh.exception.ERVHRuntimeException;
import heven.holt.lib_common.widget.refresh.view.SimpleLoadMoreView;
import heven.holt.lib_common.widget.refresh.view.SimpleRefreshHeaderView;
...
//        super.onMeasure(widthMeasureSpec, expandParentHeight);
    ...
    private void finishSpinner() {
        if (state == State.REFRESHING) {
            ...
            autoScroll.scrollTo(START_POSITION, SCROLL_TO_TOP_DURATION);
            if (!(state == State.PULL  && currentOffsetTop > totalDragDistance ))
                postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        easyEvent.endMoveRefreshing();
